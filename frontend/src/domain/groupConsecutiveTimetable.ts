// Helper to group consecutive timetable entries by subject and type with duration support
...
// Check if two time slots are consecutive using duration
function areConsecutive(first: TimetableEntry, second: TimetableEntry): boolean {
  // If duration is available, use it to calculate if they're consecutive
  if (first.duration) {
    const firstEndMinutes = timeToMinutes(first.startTime) + first.duration;
    const secondStartMinutes = timeToMinutes(second.startTime);
    return firstEndMinutes === secondStartMinutes;
  }
  // Fallback to comparing end time with start time
