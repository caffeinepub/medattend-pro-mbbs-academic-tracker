// Subject statistics card with Theory/Practical metrics and AETCOM breakdown

import { useMemo } from 'react';
import type { SubjectName, AttendanceStats, ClassLog } from '../storage/models';
...
export default function SubjectStatsCard({ subject, theoryStats, practicalStats, logs }: SubjectStatsCardProps) {
  ...
  // Calculate AETCOM-specific stats
  const aetcomStats = useMemo(() => {
    const aetcomLogs = logs.filter((log) => log.subject === subject && log.type === 'AETCOM');
    const conducted = aetcomLogs.length;
    const attended = aetcomLogs.filter((log) => log.attendanceStatus === 'Present').length;
    ...
    <div className="bg-card border border-border rounded-lg p-5 shadow-sm">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2 pt-3 border-t border-border">
          ...
          {/* AETCOM breakdown */}
          {aetcomStats.conducted > 0 && (
            <div className="pl-3 py-1 border-l-2 border-muted">
              <div className="flex items-center justify-between text-xs text-muted-foreground">
                <span>AETCOM</span>
                <span>{aetcomStats.attended} / {aetcomStats.conducted}</span>
              </div>
            </div>
          )}
