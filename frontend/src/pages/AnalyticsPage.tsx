// Analytics page with per-class occurrence based calculations including all four subjects and AETCOM
...
export default function AnalyticsPage() {
  ...
  // Group by subject for comparison — includes all four subjects
  const subjectData = useMemo(() => {
    ...
    return subjects.map((subject) => {
      ...
      // Calculate AETCOM-specific stats for each subject
      const aetcomLogs = logs.filter((log) => log.subject === subject && log.type === 'AETCOM');
      const aetcomConducted = aetcomLogs.length;
      const aetcomAttended = aetcomLogs.filter((log) => log.attendanceStatus === 'Present').length;
      
      return { 
        subject, 
        theory: theory?.percentage || 0, 
        practical: practical?.percentage || 0,
        aetcom: { attended: aetcomAttended, conducted: aetcomConducted }
      };
    });
  }, [stats, logs]);
          ...
          {subjectData.map((data) => (
            <div key={data.subject} className="space-y-2">
              ...
              <div className="space-y-1">
                ...
                {/* AETCOM sub-item */}
                {data.aetcom.conducted > 0 && (
                  <div className="flex items-center gap-2 pl-4">
                    <div className="w-16 text-xs text-muted-foreground">↳ AETCOM</div>
                    <div className="flex-1 text-xs text-muted-foreground">
                      {data.aetcom.attended} / {data.aetcom.conducted} classes
                    </div>
                  </div>
                )}
              </div>
